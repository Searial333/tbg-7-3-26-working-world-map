...
export const useInput = (containerRef: React.RefObject<HTMLElement>): InputState => {
  ...
  useEffect(() => {
    ...
    const calculateTouchInput = () => {
      ...
      let jump = false, roll = false, dash = false, shoot = false, throwInput = false, interact = false;
      ...
      for (const touch of activeTouches.values()) {
        ...
        // Action buttons
        const checkButton = (name: 'jump' | 'dash' | 'roll' | 'shoot' | 'throw' | 'interact') => {
        ...
        if (checkButton('interact')) interact = true;
      }
      setTouchInput({ left, right, up, down, jump, roll, dash, shoot, throw: throwInput, interact });
    };
  ...
  const currentInput: InputState = {
    ...
    interact: keys.KeyE || !!touchInput.interact,
  ...
  currentInput.interactDown = currentInput.interact && !previousInput.current.interact;
